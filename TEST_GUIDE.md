...
## **PART 1: CANDIDATE PORTAL TESTING** 🎯
...
✅ Expected - AI DISCLAIMER VISIBLE:
...
✅ Expected AFTER UPLOAD (within 5 seconds):
...
### **Test 6: Profile Data Extraction (OCR Check)** ⭐ CRITICAL
```
✅ Steps:
1. Wait 10-20 seconds after upload completes
2. Check Profile section

✅ Expected - Check ALL fields are populated:
...
✅ VERIFY OCR QUALITY:
□ Skills extracted correctly (check against actual resume)
□ Experience text is complete (not truncated weird)
□ Education detected properly
□ Years of experience calculated correctly

✅ If ANY field says "Not specified":
...
✅ Expected:
- Blue box with info icon (ℹ️)
- Text: "Profile auto-generated by AI. Verify and edit if needed."
```

---

### **Test 8: Edit Profile - Can Edit All Fields** 🆕
```
✅ Steps:
1. Click Edit icon (pencil) on profile

✅ Expected - AI DISCLAIMER IN EDIT MODE:
- Yellow box appears
- Text: "AI-extracted data. Please verify accuracy before saving."

2. Check ALL editable fields are present:
   □ Skills (textarea)
   □ Experience (large textarea, 5 rows)
   □ Years of Experience (text input) 🆕 THIS WAS MISSING BEFORE
   □ Education (textarea, 3 rows) 🆕 THIS WAS MISSING BEFORE
...
### **Test 9: Job Listing - Match Scores Calculation** ⭐ CRITICAL
```
✅ Steps:
1. Look at Available Jobs section (right side)

✅ Expected - WHILE CALCULATING:
- Blue banner: "Calculating match scores... Please wait."
- Jobs visible but scores may be loading

✅ Expected - AFTER CALCULATION (30-60 seconds max):
- Blue banner disappears
- Each job shows:
  □ Match percentage (0-100%) in colored number
  □ Color coding:
    - Green (80%+): Excellent Match
    - Yellow (60-79%): Good Match
    - Orange (40-59%): Fair Match
    - Red (0-39%): Low Match
  □ Progress bar below job details
  □ "Eligibility Score" label
...
### **Test 10: Apply Without Profile (Should Block)** 🆕
```
✅ Steps:
1. Logout
...
### **Test 11: Apply to Job - Status Updates Immediately** ⭐ CRITICAL FIX
```
✅ Steps:
1. With profile uploaded, find a job
2. Click "Apply Now"

✅ Expected - IMMEDIATELY (within 1-2 seconds):
...
✅ THIS IS THE FIX:
Before: Button stayed "Apply Now" for 10+ minutes ❌
After: Button changes to "Applied" instantly ✅
...
### **Test 12: Duplicate Application Prevention** 🆕
```
✅ Steps:
...
### **Test 13: Match Scores Stay Consistent** ⭐ CRITICAL FIX
```
✅ Steps:
...
✅ THIS IS THE FIX:
Before: Scores changed drastically on re-login ❌
After: Scores cached and consistent ✅
...
### **Test 14: Profile Missing After Refresh** ⭐ CRITICAL FIX
```
✅ Steps:
...
✅ THIS IS THE FIX:
Before: Profile disappeared, blank screen ❌
After: Retry logic with 3 attempts, clear error message ✅
...
### **Test 18: Applications Tab - No Duplicates** ⭐ CRITICAL FIX
```
✅ Steps:
...
✅ THIS IS THE FIX:
Before: Same application appeared 2-3 times ❌
After: Unique constraint prevents duplicates ✅
```

---

### **Test 19: Applications Sorted by Score** 🆕
```
✅ Steps:
1. Look at applications list
2. Check match score percentages

✅ Expected - SORTED HIGH TO LOW:
...
✅ THIS IS THE FIX:
Before: Random order ❌
After: Sorted by match score (best candidates first) ✅
```
...
✅ THIS WAS ALREADY WORKING (confirmed)
```
...
✅ Expected - AFTER SEARCH (10-30 seconds):
...
- Each result shows:
  ...
  □ AI Relevancy Score
  □ Two progress bars (vector and AI)
...
## **PART 3: EDGE CASES & ERROR SCENARIOS** ⚠️
...
1. Resume Upload → Profile Display: _______ seconds
   ✅ Should be: < 30 seconds

2. Click Apply → Button changes to "Applied": _______ seconds
   ✅ Should be: < 2 seconds (CRITICAL FIX)

3. Login → Dashboard loads: _______ seconds
   ✅ Should be: < 3 seconds

4. Job match score calculation (all jobs): _______ seconds
   ✅ Should be: < 60 seconds for 10 jobs
...
```
1. ✅ Candidate: Signup → Upload resume → Apply to job → Check "Applied" instantly
2. ✅ Admin: Login → Check applications sorted by score
3. ✅ Test forgot password on both portals
4. ✅ Edit profile - verify years & education fields present
5. ✅ Check no duplicate applications
6. ✅ Verify match scores consistent after re-login